...
const OwnersPage = () => {
  ...
  const durationPeriod = (dates: string) => {
    ...
    const ownerPurchasedYear = dates.split('-')[2] ? dates.split('-')[2] : '2017';
          ...
          ownersData.map((ref, index) => (
            <section className="p-4 border border-warning my-1" key={index}>
              ...
              <blockquote className="blockquote">
                <p><strong>Flat {ref.block} - {ref.floor}{ref.flat}</strong></p>
              </blockquote>
              <hr/>         
              ...
              <ul>
                {ref.personal.map((person) => (
                  <li key={person.totalsbua}>
                    <dl>
                      <dt>Date of registration:</dt><dd> {person.registrationdate}</dd>
                      <dt>Flat area:</dt><dd>  {person.totalsbua} sq. feet</dd>
                      <dt>Parking:</dt><dd>  {
                        ref.parking.available ? (
                          <>
                          <span>{ref.parking.status} | {ref.parking.aloted}</span>
                          </>
                        ) : (
                          <em>No</em>
                        )
                      }</dd>
                      <dt>Sell:</dt><dd> {person.wanttosell ? 'Yes': 'No'}</dd>
                    </dl>
                  </li>
                ))}
              </ul>
              ...
              <p className="text-danger"><small>This zone is for public so all data you shared or plan to share, please keep in mind.</small></p>          
              </section>
